...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def guided_p_sample(self, guide, x, mask, t, clip_denoised=True, repeat_noise=False, cond_idx=0, stack_idx=0, put_place=[0., 0.]):
        b, *_, device = *x.shape, x.device

        # colors = ["Red", "Green", "Blue", "Yellow"]
        # print("Stack Index: " + colors[stack_idx])
        ...
        with torch.enable_grad():
            ...
            # place_idx = (stack_idx + 1) % 4
            # place_xy = x[..., 64:, 7+place_idx*8:9+place_idx*8]
            # pdb.set_trace()

            # stack_rot = x[..., 64:, 10+stack_idx*8:14+stack_idx*8]
            # put_rot = torch.Tensor([-7.3e-02, -7.3e-02, -7.3e-02, 1]).to(device)
            # place_rot = put_rot.repeat(1,32,1)

            put_place = torch.from_numpy(put_place).reshape((1,1,2)).to(device)
            place_xy = put_place.repeat(1,64,1)  # 64

            # dist = -0.1 * torch.abs(stack_xy - place_xy).mean(dim=-1).mean(dim=-1)
            dist = -100. * torch.abs(stack_xy - place_xy).mean(dim=-1).mean(dim=-1)  # TODO: Important!! -100  -50

            # dist_rot = -25. * torch.abs(stack_rot - place_rot).mean(dim=-1).mean(dim=-1)

            # stack_xy = x[..., 96:, 7 + stack_idx * 8:9 + stack_idx * 8]
            # place_xy = put_place.repeat(1, 32, 1)
            # dist2 = -25. * torch.abs(stack_xy - place_xy).mean(dim=-1).mean(dim=-1)
        ...
        scalar = 1.  # TODO 0.7
        # pdb.set_trace()
        return model_mean + sigma * noise + sigma * grad * scalar, 0.

    @torch.no_grad()
    def guided_p_sample_loop(self, guide, x, mask, cond_idx, stack_idx, put_place):
        device = self.betas.device
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps))):
            img, values = self.guided_p_sample(guide, img, mask, torch.full((b,), i, device=device, dtype=torch.long), cond_idx=cond_idx, stack_idx=stack_idx, put_place=put_place)
            # img = self.p_sample(img, mask, torch.full((b,), i, device=device, dtype=torch.long))
            img[mask.bool()] = x[mask.bool()].clone()
    ...
    @torch.no_grad()
    def guided_conditional_sample(self, guide, batch_size, conditions, cond_idx, stack_idx, put_place):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
        ...
        for t, cond, val in conditions:
            x[:,t,:] = val
            mask[:, t] = 1
        my_val = conditions[0][2].clone()
        # pdb.set_trace()
        my_val[..., 7+stack_idx*8:9+stack_idx*8] = torch.from_numpy(put_place).to(device)  # only change set_block's place
        x[:, -1, :] = my_val
        mask[:, -1] = 1
        # print("Set well")
        return self.guided_p_sample_loop(guide, x, mask, cond_idx, stack_idx, put_place)
