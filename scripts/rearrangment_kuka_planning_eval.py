...
def eval_episode(model, env, dataset, idx=0):
    ...
    for i in range(3):
        stack, place = env.stacks[env.progress]
    ...
    if not osp.exists("rearrange_samples/"):
        os.makedirs("rearrange_samples/")

    writer = get_writer("rearrange_samples/cond_video_writer{}.mp4".format(idx))

    for frame in frames:
        writer.append_data(frame)

    np.save("rearrange_samples/cond_sample_{}.npy".format(idx), np.array(total_samples))
...
env = StackEnv(conditional=False, rearrangment=True)
