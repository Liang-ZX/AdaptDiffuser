...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if type(folder) == str:
            ...
            self.ds = folder
        ratio = 1 / 2 if old_dataset is not None else 1
        self.dl = cycle(data.DataLoader(self.ds, batch_size=int(train_batch_size * ratio), shuffle=True, pin_memory=True))
        self.renderer = renderer
        self.opt = Adam(diffusion_model.parameters(), lr=train_lr)

        # TODO
        self.old_ds = old_dataset
        self.old_dl = cycle(data.DataLoader(self.old_ds, batch_size=int(train_batch_size * (1 - ratio)), shuffle=True,
                                            pin_memory=True)) if old_dataset is not None else None
    ...
    def load(self, milestone=-1, loadpath=None):
        if loadpath is None:
            assert milestone > 0
            data = torch.load(str(self.results_folder / f'model-{milestone}.pt'))
            self.step = data['step']
        else:
            # os.path.join(loadpath, f'model-{milestone}.pt')
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                ...
                # TODO: mix training
                if self.old_dl is not None:
                    old_data, old_mask = next(self.old_dl)
                    data = torch.cat([data, old_data], dim=0)
                    mask = torch.cat([mask, old_mask], dim=0)
            ...
            if self.step == 0 and self.visualization:
